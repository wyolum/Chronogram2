...
sys.path.append(os.path.join(install_dir, 'fabricate', 'CNC'))
from cnc import *
from baffles import *
...
fontpath = ['/Users/Nick/Dropbox/WyoLumCode/fonts/', '/Users/justin/Documents/googlefontdirectory/']
...
def add_font(fontname, path=None):
            ...
            def addit(args, d, names):
                ...
                for fn in names:
                    ...
                    if FN[:-4] == FONTNAME and FN[-4:] == '.TTF':
                        ...
                        if not os.path.exists(os.path.join('/Users/Nick/Dropbox/WyoLumCode/fonts/', fn)):
                            shutil.copy(os.path.join(d, fn), os.path.join('/Users/Nick/Dropbox/WyoLumCode/fonts/', fn))
                        break
    ...
    if FONTNAME not in fontnames:
        print fontnames
        raise ValueError("could not install FONTNAME: %s" % FONTNAME)
...
def getPCB(outline=False, leds=False, buttons=False, button_holes=True):
    ...
    for i in range(5):
        ...
        if button_holes:
            button_hole(x, y, pcb)
            # pcb.addText(x, y + .25 * inch, labels[i], 'TERMINALDOSIS-MEDIUM', 12, color=pink)    
    ## thumbwheel
    x = pcb_w - .5 * inch - 5 * .6 * inch
    y = pcb_h - .5 * inch
    pcb.drill(x, y, .25 * inch)
    # pcb.addText(x, pcb_h - .4 * inch + .25 * inch, 'DIM', 'TERMINALDOSIS-MEDIUM', 12, color=pink)
    pcb.rect([pcb_w - .44 * inch - .62 * inch, .44 * inch,
              .62 * inch, .12 * inch])
    x = pcb_w - .44 * inch - .62 * inch
    y = .28 * inch
    # can.drawCentredString(y - .04 * inch, -x,'GRN')
    # pcb.addText(x, y, 'G', 'TERMINALDOSIS-MEDIUM', 12, color=pink)
    # can.drawCentredString(y, -x - .75 * inch,'BLK')
    # pcb.addText(x + .65 * inch, y, 'B', 'TERMINALDOSIS-MEDIUM', 12, color=pink)
    # pcb.addText(x + .6 * inch / 2, y + .35 * inch, '5V FTDI', 'TERMINALDOSIS-MEDIUM', 12, color=pink)
    return pcb
...
langs_path = os.path.join(install_dir, 'fabricate', 'Langs')
english2_v1 = Langs.Simulate2x.readwtf(os.path.join(langs_path, "English2_v1.wtf"))['letters']
# german2_v1440 = Langs.Simulate2x.readwtf(os.path.join(langs_path, "German2_v1440.csv"))['letters']
cases = {'lower':lower,
         'UPPER':upper}
...
def main():
    ...
    # add_font('TerminalDosis-Medium', os.path.join(install_dir, 'fabricate', 'fonts'))
    # add_font('TerminalDosis-Regular', os.path.join(install_dir, 'fabricate', 'fonts'))
    add_font(font, os.path.join(install_dir, 'fabricate', 'fonts'))
    ...
    # t = MyText(15.2 * inch, .85*inch, "MASTER", centered=False, color=pink)
    # t.drawOn(bp_can)
    bp_can.save()
