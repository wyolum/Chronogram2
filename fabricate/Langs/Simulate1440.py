...
class Wordmap:
    def __init__(self, ss, start_row, start_col, n_row):
        ...
        if False:
            pl.pcolormesh(self.bitmap.T, cmap='binary_r')
            words = [str(w, 'utf-8') for w in self.words]
            pl.yticks(arange(len(words)) + .5, words, fontsize=10)
            pl.xticks(arange(0, 1440, 120), ['%d:00' % i for i in range(0, 24, 2)], rotation=90)
            pl.xlim(0, 1440)
            pl.ylim(0, len(words))
            pl.show()
    ...
    def get_line(self, idx):
        return ' '.join(w for j, w in enumerate(self.words) if
                       self.bitmap[idx][j])
    def binconvert(self, cname, outfile=None, bin=True):
        if outfile is None:
            outfile = sys.stdout
        ...
        for i in range(n_word):
            if i < l:
                ...
                ln = lens[i]
                if bin:
                    outfile.write(struct.pack('BBB', x, y, ln))
                else:
                    print('    %3d,%3d,%3d,' % (x, y, ln), end=' ', file=outfile)
            else:
                if bin:
                    outfile.write(struct.pack('BBB', 0, 0, 0))
                else:
        ...
        if bin:
            outfile.write(struct.pack('B', n_word // 8))
        else:
            print('};', file=outfile)
            print('', file=outfile)
...
def readwtf(csvfile):
    ...
    ## check version is 1440
    assert getAssignment(ss.getCell("A2"), typ=None) == "1440", "Incorrect format in cell 'A2' '%s', expected 'version=1440'" % ss.getCell('A2')

    n_row = getAssignment(ss.getCell('A4'), name_ck='n_row')
    n_col = getAssignment(ss.getCell('A5'), name_ck='n_col')
    print("ss.getCell('A6')", ss.getCell('A6'))
    data_start_row = getAssignment(ss.getCell('A6'), name_ck='data_start')
    ...
    assert ss['A%d' % data_start_row].strip().startswith('n_word=')
    ...
    wordmap = Wordmap(ss, data_start_row - 1, 0, 1440)
    for min in range(1440):
        # print wordmap.get_line(min)
        pass
...
class ClockTHREEjr:
    ...
    ##  Make menu
        ...
        filemenu.add_command(label="Bin-Convert", command=self.binconvert)
...
def binconvert(filename, n_row=8, outfilename=None):
    data = readwtf(filename)
    if outfilename is None:
        outfile = sys.stdout
    else:
        outfile = open(outfilename, 'w')
    data['min_wordmap'].binconvert('MINUTE', outfile, outfilename.endswith('.bin'))
...
def readbin(fn):
    s = open(fn).read()
    vals = struct.unpack('%dB' % len(s), s)
    n_word = vals[0]
    print(n_word)
    for i in range(n_word):
        for j in range(3):
            print('    ',  vals[i * 3 + 1 + j], end=' ')
        print()
    print()
    n_byte = vals[n_word * 3 + 1]
    offset = n_word * 3 + 2
    print(n_byte, '// number of bytes per state')
    for i in range(1440):
        hh, mm = divmod(i, 60)
        print('%02d:%02d' % (hh, mm), end=' ')
        for j in range(n_byte):
            print(uint8_2_bits(vals[offset +  i * n_byte + j]), end=' ')
        print()
# readbin('junk.bin'); here
# readbin('G2_V1440.bin'); here
